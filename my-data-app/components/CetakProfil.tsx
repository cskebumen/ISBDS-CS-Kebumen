...
export const CetakProfil = React.forwardRef<HTMLDivElement, CetakProfilProps>(({ data, riwayat }, ref) => {
  ...
  return (
    <div ref={ref} className="p-12 bg-white text-black font-serif print:p-8" style={{ width: '210mm', minHeight: '297mm' }}>
      ...
      <div className="space-y-2 text-sm mb-8 ml-4">
        <div className="flex"><div className="w-64 font-bold">1. Nama Anggota</div><div>: {data.nama_lengkap || '-'}</div></div>
        <div className="flex"><div className="w-64 font-bold">2. Nomor Induk Anggota (NIA)</div><div className="font-mono">: {data.nia || '-'}</div></div>
        <div className="flex"><div className="w-64 font-bold">3. Tempat, Tanggal Lahir</div><div>: {data.tempat_lahir || '-'}, {data.tanggal_lahir || '-'}</div></div>
        <div className="flex"><div className="w-64 font-bold">4. Jenis Kelamin</div><div>: {data.jenis_kelamin || '-'}</div></div>
        <div className="flex"><div className="w-64 font-bold">5. Alamat Lengkap</div><div className="flex-1">: {data.alamat_lengkap || '-'}</div></div>
        <div className="flex"><div className="w-64 font-bold">6. Ranting / Unit</div><div>: {data.ranting || '-'}</div></div>
        <div className="flex"><div className="w-64 font-bold">7. Cabang</div><div>: KEBUMEN</div></div>
        <div className="flex"><div className="w-64 font-bold">8. No. HP / WhatsApp</div><div>: {data.no_hp || '-'}</div></div>
      </div>
      ...
      <table className="w-full border-collapse border border-black text-sm mb-12">
        ...
        <tbody>
          {riwayat.length > 0 ? riwayat.map((r, i) => (
            <tr key={i} className="text-center">
              <td className="border border-black p-2">{i + 1}</td>
              <td className="border border-black p-2 text-left px-4 italic font-semibold">{r.tingkat}</td>
              <td className="border border-black p-2">{r.no_sertifikat}</td>
              <td className="border border-black p-2">{r.tahun}</td>
            </tr>
          )) : (
            <tr><td colSpan={4} className="border border-black p-4 text-center italic text-gray-500">Data riwayat belum tersedia</td></tr>
          )}
        </tbody>
      </table>
      ...
      <div className="mt-16 pt-4 border-t border-gray-200 flex items-center justify-between">
        ...
        <p className="text-[9px] text-gray-400 text-right leading-tight">
          Dokumen ini dihasilkan secara otomatis oleh<br/>
